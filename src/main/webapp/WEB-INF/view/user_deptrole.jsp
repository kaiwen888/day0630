...
<body>
    <form action="saveUserDeptRole.do" method="post">
        ...
        <h1>目前正在给---${user.username}---分配部门和角色</h1>
        分配部门:<select name="deptid" onchange="getrolelist(this)">
                    <option value="-1">--请选择部门--</option>
                    <c:forEach var="d" items="${dlist}">
                        <option value="${d.deptid}" ${user.deptBean.deptid==d.deptid?'selected':''}>${d.dname}</option>
                    </c:forEach>
                </select>----
        分配角色:<select name="rid">
                    <option value="-1">--请选择角色--</option>
                    <c:forEach var="r" items="${rlist}">
                        <option value="${r.rid}" ${user.roleBean.rid==r.rid?'selected':''}>${r.rname}</option>
                    </c:forEach>
            </select>
